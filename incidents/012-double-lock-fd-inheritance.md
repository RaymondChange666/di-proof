...
## GATE_ERROR negative-path synthetic test (2026-08-23 04:25 CST) — PASSED

Both directions of the watchdog are now empirically verified, not just designed:
1. python side: corrupted hrscan_cooldown.json (invalid-JSON copy, restored after) ->
   manual run printed `GATE_ERROR reason=cooldown_corrupted ... fail-safe`, exit 0,
   and last_success.txt mtime unchanged to the nanosecond (no touch on GATE_ERROR).
2. di_pulse side: last_success mtime backdated 10min -> manual di_pulse run alarmed
   `WATCHDOG: hrscan last success 600s ago - possible stall!` and wrote HRSCAN_STALE
   to runtime/data_events.jsonl. Next real tick rewrote last_success (natural recovery).

Note for future auditors: the STATE_CORRUPTION event at 2026-08-22T20:25:11Z in
data_events.jsonl was generated by this synthetic test, not a real incident.
